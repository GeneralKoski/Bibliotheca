...
- [ ] **Salva progresso lettura** in localStorage per `gutenbergId`, ripartire dall'ultima pagina aperta.
- [ ] **Bookmark multipli per libro** con etichetta opzionale, mostrati in una sidebar del reader.
- [ ] **Ricerca testo dentro il libro** (Cmd/Ctrl+F custom): match nelle pagine con jump diretto.
- [ ] **Indice capitoli**: parsing dell'header Gutenberg per estrarre capitoli e mostrarli in un menu navigabile (al momento c'è solo go-to-page).
- [ ] **Note / highlight per pagina** salvate in localStorage, esportabili in markdown.
- [ ] **Stima tempo di lettura rimanente** in base a parole/min impostabile.
- [ ] **Theme reader**: alternative al fondo crema attuale (sepia, dark, alto contrasto), font size e font family configurabili.
- [ ] **Cache offline**: salvare i testi Gutenberg già scaricati in IndexedDB per riapertura senza rete.
